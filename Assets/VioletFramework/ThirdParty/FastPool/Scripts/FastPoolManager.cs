...
public class FastPoolManager : MonoBehaviour
{
    ...
    [SerializeField]
    List<FastPool> predefinedPools = null;
    ...
    /// <param name="warmOnLoad">Load source prefab in the memory while scene is loading. A bit slower scene loading, but much faster instantiating of new objects in pool</param>
    /// <returns>FastPool instance</returns>
    public static FastPool CreatePool(int id, GameObject prefab, bool warmOnLoad = true, int preloadCount = 0, int capacity = 0, PoolAreaType areaType = PoolAreaType.Everlasting)
    {
        if (prefab != null)
        {
            if (!Instance.pools.ContainsKey(id))
            {
                FastPool newPool = new FastPool(id, prefab, Instance.curTransform, warmOnLoad, preloadCount, capacity, areaType);
    ...
    public static FastPool GetPool(int id) {
        if (Instance.pools.ContainsKey(id)) {
            return Instance.pools[id];
        }
        return null;
    ...
    public static void DestroyPools(PoolAreaType areaType) {
        List<int> _clearPoolIDList = new List<int>();
        var enumer = Instance.pools.GetEnumerator();
        while (enumer.MoveNext()) {
            FastPool pool = enumer.Current.Value;
            if(pool.AreaType == areaType) {
                _clearPoolIDList.Add(pool.ID);
                pool.ClearCache();
            }
        }

        for(int i = 0; i < _clearPoolIDList.Count; ++i) {
            Instance.pools.Remove(_clearPoolIDList[i]);
        }
        _clearPoolIDList.Clear();
        _clearPoolIDList = null;
    }
