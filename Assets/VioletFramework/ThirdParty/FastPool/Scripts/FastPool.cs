...
public class FastPool
{
    ...
    /// <summary>
    /// 标识这个Pool属于哪个区域，以决定在什么时候会被销毁
    /// </summary>
    public PoolAreaType AreaType {
        get;
        private set;
    }
    ...
    /// <param name="preloadCount">How much items must be cached at the start</param>
    /// <param name="capacity">Cache size (maximum amount of the cached items)  [0 - unlimited]</param>
    public FastPool(int id, GameObject prefab, Transform rootTransform = null, bool warmOnLoad = true, int preloadCount = 0, int capacity = 0, PoolAreaType areaType = PoolAreaType.Everlasting)
    {
        useCustomID = true;
        customID = id;
        sourcePrefab = prefab;
    ...
    GameObject MakeClone(Vector3 position, Quaternion rotation, Transform parent)
    {
        ...
        GameObject clone = (GameObject)GameObject.Instantiate(sourcePrefab);
